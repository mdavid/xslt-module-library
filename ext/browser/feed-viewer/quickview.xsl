...
<xsl:stylesheet version="1.0" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:dc="http://purl.org/dc/elements/1.1/"
  ...
  <xsl:template match="/">
    ...
    <ul class="viewer" style="float:left;width:100%;margin:0px 0px 0px 0px;z-index:0;font-variant:small-caps;padding:5px 0px 5px 0px;border-top:2px
      solid #588099;">
      <li style="width:100%;float:left;clear:both;text-align:center;padding:1px 0px 1px 0px;margin:0px 0px 5px 0px;border-top:1px solid
        #558099;border-bottom:1px solid #558099;">
        <ul style="padding:0px;font-size:small;text-align:left;color:#588099;margin:0px 1px 0px 1px;border-right:3px solid #EEEDE9;border-left:3px
          solid #EEEDE9;">
          ...
          <li style="float:left;clear:none;font-size:medium">&#160;of <xsl:value-of select="$entryCount"/></li>
          <li style="float:right;clear:none;width:17%">
            <ul style="float:right">
              <xsl:if test="$pos >= 30 and ($entryCount + $pos) > 30">
                ...
                <li onclick="SetParam('pos',{$pos - $stepCount});SetParam('feed','{$feed}');initialiseTransform('dataFeed', loadXMLFeed('{$feed}'),
                  '/quickview.xsl', '{$feed}QF{$pos - $stepCount}');return false;" style="border:1px solid
                  #588099;width:auto;height:18px;text-align:center;color:#588099;font-weight:600;padding:2px;margin:1px;cursor:pointer;float:left;text-align:center;"
              ...
              <xsl:if test="$pos &gt; $stepCount">
                <li onclick="SetParam('pos',{$posStart - $stepCount});SetParam('feed','{$feed}');initialiseTransform('dataFeed',
                  loadXMLFeed('{$feed}'), '/quickview.xsl', '{$feed}QF{$pos -  $stepCount}');return false;" style="border:1px solid
                  #588099;width:20px;height:18px;text-align:center;color:#588099;font-weight:600;padding:2px;margin:1px;cursor:pointer;float:left;text-align:center;"
                    >-<xsl:value-of select="$stepCount"/>
                </li>
              </xsl:if>
              <li onclick="SetParam('pos',{$posStart + $stepCount});SetParam('feed','{$feed}');initialiseTransform('dataFeed', loadXMLFeed('{$feed}'),
                '/quickview.xsl', '{$feed}QF{$pos + $stepCount}');return false;" style="width:20px;height:18px;border:1px solid
                #588099;text-align:center;color:#588099;font-weight:600;padding:2px;margin:1px;cursor:pointer;float:left;text-align:center;"
                  >+<xsl:value-of select="$stepCount"/>
              </li>
            </ul>
          </li>
  ...
  <xsl:template match="atom:entry">
    ...
    <a href="{atom:link/@href}">
      <li class="feedEntry" style="z-index:20;margin-left:{$leftMargin}px;color:#FFF" onmouseover="this.style.color = '#588099';this.style.background
        = '#FFF';this.style.borderColor
        = '#588099';return true;" onmouseout="this.style.color = '#FFF';this.style.background = '#588099';this.style.borderColor
        = '#FFF';return true;">
        <ul>
          <li class="qvTitle">
  ...
  <xsl:template match="atom:entry" mode="calLinearGrid">
    ...
    <li style="float:left;width:20px;height:18px;border:1px solid #{substring-before($color,
      '-')};background-color:#{substring-after($color,'-')};text-align:center;padding:2px;margin:1px;color:#{substring-before($color, '-')}">
