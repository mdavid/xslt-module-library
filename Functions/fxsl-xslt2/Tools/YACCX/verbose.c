/* $Id: verbose.c,v 1.1 2007/05/08 13:10:38 dnovatchev Exp $ */
...
static void print_state_xml(int state);
static void print_gotos_xml(int stateno);
static void print_symbols_xml();
...
{
   char* retstr = (char*) calloc(strlen(str) + 1, sizeof(char));
...
static void print_state_xml(int stateno)
{
    register int count;
    register action *q;
	action *p = parser[stateno];
    register int k, anyreds;
	register int defred2 = defred[stateno];

	count = 0;

	fprintf(xml_file, "\n	  <state number=\"%d\">", stateno);
    ...
    if (count > 0)
    {
		for (; p; p = p->next)
		{
			if (p->action_code == SHIFT && p->suppressed == 0)
				fprintf(xml_file, "\n	    <symbol act=\"s%d\">%s</symbol>",
						p->number, strip_quotes(symbol_name[p->symbol]));
		}
    }
    ...
    if(anyreds > 0)
	{
		for (; p; p = p->next)
		{
			...
			if (p->suppressed == 0)
				fprintf(xml_file, 
						"\n	    <symbol act=\"r%d\">%s</symbol>",
						k, strip_quotes(symbol_name[p->symbol]));
	...
	if (defred2 > 0)
		fprintf(xml_file, 
				"\n	    <symbol act=\"r%d\">.</symbol>", defred2 - 2);
...
static void print_reductions(register action *p, register int defred2)
{
    register int k, anyreds;
    register action *q;
...
static void print_gotos_xml(int stateno)
{
	...
	if(sp)
	{
		...
		for (i = 0; i < sp->nshifts; ++i)
		{
			...
			if (ISVAR(as))
				fprintf(xml_file, 
						"\n	  <state number=\"%d\" NT=\"%s\" newState=\"%d\"/>", 
						stateno, strip_quotes(symbol_name[as]), k);
		}
	}
}
...
static void print_symbols_xml()
{
  //nsyms, ntokens, nvars
	...
	fprintf(xml_file, "\n	  <terminal>");
		for(i = 0; i < nsyms; i++)
		{
			//as = accessing_symbol[i];

			if(ISTOKEN(i))
				fprintf(xml_file, "\n	    <s>%s</s>", strip_quotes(symbol_name[i]));
		}
	fprintf(xml_file, "\n	  </terminal>");
	...
	fprintf(xml_file, "\n	  <nonTerminal>");
		for(i = 0; i < nsyms; i++)
		{
			...
			if(! (ISTOKEN(i)) )
				fprintf(xml_file, "\n	    <v>%s</v>", strip_quotes(symbol_name[i]));
		}
	fprintf(xml_file, "\n	  </nonTerminal>");
